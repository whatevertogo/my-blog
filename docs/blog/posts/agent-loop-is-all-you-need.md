...
大家都在想claude code为什么如此牛博弈，为什么能在智能体领域表现出色。其实，答案很简单：agent loop 就是你需要的所有内容了。


## 什么是 Agent Loop？

Agent Loop 是一种基于循环的智能体架构，它允许智能体在不断地与环境交互中选择不同决策和使用工具。通过不断地观察环境、做出决策、执行动作并获取反馈，智能体能够逐步优化自己的行为策略，从而在各种任务中表现出色。
