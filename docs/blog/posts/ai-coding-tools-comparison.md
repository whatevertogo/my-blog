...
tags:
  ...
  - Kimi CLI
  - Pi Mono
description: "从 grep 的上下文支持到 edit 的模糊匹配，从 shell 超时到图片读取，深度对比 Astocode(Codex)、Claude Code、OpenCode、Codex、Pi-mono 和 Kimi-cli 六大编程 AI 的工具系统实现细节。"
---
...
本文聚焦 **Grep、Read、Edit、Shell、Find/List** 这六大核心工具类别，基于对 [Astrocode(Codex)](https://github.com/openai/codex)、[Claude Code](https://github.com/anthropics/claude-code)、[OpenCode](https://github.com/sst/openchakra)、[Pi-mono](https://github.com/pi-corp/pi-mono) 和 [Kimi CLI](https://github.com/MoonshotAI/kimi-cli) 的源码分析，给出客观的能力对比与技术细节。

## 一、总览对比表

| 能力 | Astocode | Claude Code | OpenCode | Codex | Pi-mono | Kimi-cli |
|------|----------|-------------|----------|-------|---------|----------|
| **grep 后端** | regex+walkdir | ripgrep | ripgrep | ripgrep | ripgrep | ripgrep |
| **grep glob 过滤** | ✘ | ✔ | ✔ | ✔ | ✔ | ✔ |
...
| **listDir 元数据** | name/type | name/type | name only | name/type | name only | size/mode/name |
| **多参数列** | ✘ | ✘ | ✘ |  | ✔ | ✘ |
...
基于 Rust 的 `regex + walkdir` 组合，**没有使用 ripgrep**。这导致它缺少很多现代 grep 的特性：

- 不支持 glob 过滤，无法按 `*.ts` 这样的模式筛选文件
- 不支持上下文行（-B/-A/-C），搜索结果只有文件路径
- 没有任何文件类型过滤能力

```rust
// codex-rs/core/src/tools/handlers/grep_files.rs
// 核心参数仅有 pattern, include(glob), path, limit
struct GrepFilesArgs {
    pattern: String,
    include: Option<String>,      // 仅支持 glob 过滤
    path: Option<String>,
    limit: usize,                 // 默认 100，最大 2000
}
...
**关键缺陷**：无法在搜索结果中直接看到匹配行的内容和上下文，AI 需要额外调用 read 工具才能获取上下文信息，这增加了 token 消耗和响应延迟。
...
最值得关注的是 **Astrocode**，虽然它的基础功能（grep 上下文、行号、模糊匹配）相对简陋，但它的 `grep_files`/`read_file`/`list_dir` 仍作为实验性工具存在，说明项目仍在持续演进中。
